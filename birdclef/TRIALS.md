...
| 057 | rank_average_blend | **49** | - | 채점중 | 앙상블 *fusion 연산자* 변경 (ralph reflection it.6 제안 #4): cell 65의 가중 logit 합 → 클래스별 percentile rank의 가중 평균. 각 컴포넌트(Perch 72%·EffNet5fold 15%·mwf0 5%·pmix 8%)를 클래스별로 rankdata→[0,1] 변환 후 같은 weight로 평균 → `logit(p)`로 logit 공간 복원(downstream 온도/file-level/rank-aware/delta-shift/prior-mask 파이프라인 무변경). logit 공간에선 다이내믹 레인지 큰 컴포넌트가 명목 weight보다 더 기여 → rank-avg는 정확히 명목 weight만큼만 ordering 기여. 모델·weight·후처리 전부 trial_056 동일. kernel v44, wall 214.0s, submission mean 0.2476(rank percentile이 0.5 근처 → mean 상승 정상, macro AUC scale-invariant) | 🔄 채점 대기 (ralph it.7) |
