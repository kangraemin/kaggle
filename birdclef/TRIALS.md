...
| 058 | logit_restore | **50** | - | 채점중 | trial_057(-0.005) 회귀 청소 — cell 65를 `2c2a948^`(=trial_056 0.934) 버전으로 되돌려 logit-space 가중 합 fusion 복원. rank-average 코드(scipy.rankdata 루프) 제거, `final = _perch*(1-Σw) + 0.15*effnet5fold + 0.05*mwf0 + 0.08*pmix` 복귀. 모델·weight·후처리·kernel-metadata 전부 trial_056 동일 (logit-blend 코드 자체는 byte-identical, 코멘트/print만 갱신). kernel v45, wall 136.1s, submission mean 0.0425 (= trial_056 동일값 → 복원 검증). ralph reflection it.7 + sub_49 reflection 둘 다 '[필수] 다음 trial = trial_056 logit-space 블렌드 복원' 명시 | 🔄 채점중 (ralph it.8, 0.934 회귀 기대) |
