...
| 061 | convnext_5fold | **53** | - | 채점중 | trial_060 ConvNeXt fold0 single 3% (TIMEOUT, 점수 미확정)의 ConvNeXt 컴포넌트를 fold0..4 5-fold 평균으로 확장. 동일 3% weight 유지(BLEND_CONVNEXT=0.03), 다른 컴포넌트·prior_mask·fusion 식 전부 trial_060 고정 (Perch 72%/EffNet5fold 15%/mwf0 2%/pmix 8%/distill 0%). pmix fold 확장 패턴(trial_051 2-fold neutral → trial_053 4-fold +0.001) 모방 — single-fold noise 너머의 ConvNeXt 신호 증폭 시도 + trial_060 timeout 복구. Cell 65 변경: `_ck_path = ...best_fold0.pth` → `sorted(glob(best_fold*.pth))`, 단일 `_convnext_model` → `_convnext_models[]` 리스트, 5개 fold per-fold `ConvNextForImageClassification.from_pretrained + load_state_dict(strict=True)`(backbone 참조 공유 방지), inference에서 `np.stack([m(_spec) for m in _convnext_models]).mean(axis=0)` (pmix 패턴 그대로 차용). Cell 66 trial-label만 trial_060→trial_061. dataset_sources/kernel id 변경 없음. kernel v48 push 완료. Wall 예상 ≈ trial_060 445s + (5-fold avg 추가 시간 ~7-8min, fold0 단일 1.8min → 5x 9min) = 12-13min 총. 메모리 5×350MB ckpts ≈ 1.75GB CPU 여유. | 🔄 PENDING (ralph it.11, ConvNeXt 컴포넌트 fold 확장 + trial_060 timeout 복구 동시 시도) |
