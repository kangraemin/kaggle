...
| 056 | pmix_weight_up | **48** | - | 채점중 | trial_055 회귀 → 15% 슬롯 epoch50 SoftAUC 5ckpt 단일로 복원(trial_054 0.934 구성). 그 위에서 sub_45 reflection 제안대로 BLEND_PSEUDOMIX 0.06→0.08, 2pp는 가장 노이즈 많은 mwf0 fold0에서 빼옴(BLEND_MWF0 0.07→0.05). Perch 72% 고정(trial_036 Perch↓ 역효과 회피). EffNet 예산 28% 안에서 단일 fold0 → 5-fold 앙상블 pmix 쪽으로 재배분 격리. kernel v43, wall 203.6s, submission mean 0.0425 | 🔄 채점중 |
