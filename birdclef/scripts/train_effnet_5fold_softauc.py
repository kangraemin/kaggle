"""EfficientNetV2-B0 5-fold training with Perch distillation + SoftAUC hybrid loss.

Changes over train_effnet_5fold_distill.py:
- SoftAUCLoss: pairwise ranking loss that directly approximates per-class AUC
- loss_cls = 0.5 * BCE + 0.5 * SoftAUC (hybrid for stability)
- Distillation loss unchanged (DISTILL_ALPHA=0.3)
"""
import os
import random
import numpy as np
...
OUT = Path(__file__).parent.parent / 'models' / 'effnet_5fold_softauc'
