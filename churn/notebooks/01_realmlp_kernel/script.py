...
"""
import subprocess
subprocess.check_call(["pip", "install", "pytabkit", "--no-deps", "-q"])
subprocess.check_call(["pip", "install", "delu", "rtdl_num_embeddings", "-q"])
...
SEED = 42; N_FOLDS = 5
import os, glob
# competition data 경로 자동 탐색
candidates = [
    "/kaggle/input/playground-series-s6e3",
    "/kaggle/input/predict-customer-churn",
...
if DATA_DIR is None:
    ...
    print(f"Available inputs: {inputs[:20]}")
    for d in inputs:
