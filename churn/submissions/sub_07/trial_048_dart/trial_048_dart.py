...
OUT_DIR  = f"{DATA_DIR}/submissions/sub_07/trial_048_dart"
...
# DART boosting
params = {"objective":"binary","metric":"auc","boosting_type":"dart",
...
for seed in SEEDS:
    ...
    for fold,(tr_idx,val_idx) in enumerate(skf.split(train[num_cols],y)):
        ...
        dtrain = lgb.Dataset(X_tr, label=y.iloc[tr_idx])
        dval   = lgb.Dataset(X_val, label=y.iloc[val_idx], reference=dtrain)
        # DART는 early stopping 안 쓰고 고정 iterations
        model = lgb.train({**params, "random_state": seed}, dtrain,
                          num_boost_round=500, valid_sets=[dval],
                          callbacks=[lgb.log_evaluation(-1)])
        seed_oof[val_idx] = model.predict(X_val)
        seed_test += model.predict(X_te) / N_FOLDS
...
pd.DataFrame({"id":test["id"],"Churn":all_test}).to_csv(f"{OUT_DIR}/trial_048_dart.csv",index=False)
json.dump({"id":"048","status":"done","val_score":round(oof_auc,5),
           "notes":"LGBM DART + reg=2.0 + smoothed TE + 7seeds"},
