...
OUT_DIR  = f"{DATA_DIR}/submissions/sub_07/trial_038_xgb_reg5"
...
params = {"objective":"binary:logistic","eval_metric":"auc","learning_rate":0.05,
          ...
          "reg_alpha":5.0,"reg_lambda":5.0,"gamma":1.0,"verbosity":0}
...
pd.DataFrame({"id":test["id"],"Churn":all_test}).to_csv(f"{OUT_DIR}/trial_038_xgb_reg5.csv",index=False)
json.dump({"id":"038","status":"done","val_score":round(oof_auc,5),
           "notes":"XGB reg_alpha=5.0 + reg_lambda=5.0 + max_depth=4 + 7seeds"},
