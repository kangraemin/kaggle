"""LGBM num_leaves=2 (depth~1) multi-seed"""
import pandas as pd, numpy as np, lightgbm as lgb, json, warnings
...
OUT_DIR=f"{DATA_DIR}/submissions/sub_07/trial_059_lgbm_depth1"
...
y=train["target"]
params={"objective":"binary","metric":"auc","learning_rate":0.05,"num_leaves":2,"min_child_samples":50,"feature_fraction":0.8,"bagging_fraction":0.8,"bagging_freq":5,"lambda_l1":1.0,"lambda_l2":1.0,"verbose":-1}
...
for seed in SEEDS:
    ...
    for fold,(tr_idx,val_idx) in enumerate(skf.split(train[num_cols],y)):
        ...
        model=lgb.LGBMClassifier(**params,n_estimators=2000,random_state=seed)
        model.fit(X_tr,y.iloc[tr_idx],eval_set=[(X_val,y.iloc[val_idx])],callbacks=[lgb.early_stopping(50,verbose=False),lgb.log_evaluation(-1)])
...
pd.DataFrame({"id":test["id"],"Churn":all_test}).to_csv(f"{OUT_DIR}/trial_059_lgbm_depth1.csv",index=False)
json.dump({"id":"059","status":"done","val_score":round(oof_auc,5)},open(f"{OUT_DIR}/results.json","w"),indent=2)
print("Done.")
