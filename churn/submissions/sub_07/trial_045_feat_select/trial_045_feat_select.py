...
OUT_DIR  = f"{DATA_DIR}/submissions/sub_07/trial_045_feat_select"
...
# 1단계: feature importance 계산 (seed=42, 3-fold)
...
for tr_idx,val_idx in skf3.split(train[num_cols],y):
    ...
    m = lgb.LGBMClassifier(n_estimators=300, verbose=-1, random_state=42)
    m.fit(X_tr, y.iloc[tr_idx])
    if fi_total is None: fi_total = np.zeros(len(X_tr.columns)); feat_names = list(X_tr.columns)
    fi_total += m.feature_importances_

fi = pd.Series(fi_total, index=feat_names).sort_values(ascending=False)
top_feats = list(fi.head(15).index)
print(f"Top 15 features: {top_feats[:5]}...")

# 2단계: top 15만 사용해서 multi-seed
all_oof = np.zeros(len(train)); all_test = np.zeros(len(test))
params = {"objective":"binary","metric":"auc","learning_rate":0.05,"num_leaves":31,
          "min_child_samples":50,"lambda_l1":2.0,"lambda_l2":2.0,"verbose":-1}
...
for seed in SEEDS:
    ...
    for fold,(tr_idx,val_idx) in enumerate(skf.split(train[num_cols],y)):
        ...
        X_tr  = pd.concat([tr_df[num_cols].reset_index(drop=True),  pd.DataFrame(te_tr)],  axis=1)[top_feats]
        X_val = pd.concat([val_df[num_cols].reset_index(drop=True), pd.DataFrame(te_val)], axis=1)[top_feats]
        X_te  = pd.concat([test[num_cols].reset_index(drop=True),   pd.DataFrame(te_te)],  axis=1)[top_feats]
...
pd.DataFrame({"id":test["id"],"Churn":all_test}).to_csv(f"{OUT_DIR}/trial_045_feat_select.csv",index=False)
json.dump({"id":"045","status":"done","val_score":round(oof_auc,5),
           "top_features":top_feats,"notes":"top 15 features only + strong reg + 7seeds"},
