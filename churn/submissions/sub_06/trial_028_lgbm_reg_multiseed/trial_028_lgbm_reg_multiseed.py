...
OUT_DIR  = f"{DATA_DIR}/submissions/sub_06/trial_028_lgbm_reg_multiseed"
...
print(f"\nLGBM Reg Multi-seed OOF AUC: {final_auc:.5f}")
np.save(f"{OUT_DIR}/oof_preds.npy", all_oof)
np.save(f"{OUT_DIR}/test_preds.npy", all_test)
pd.DataFrame({"id":test["id"],"Churn":all_test}).to_csv(f"{OUT_DIR}/trial_028_lgbm_reg_multiseed.csv",index=False)
json.dump({"id":"028","status":"done","val_score":round(final_auc,5),"seeds":SEEDS,
           "notes":"강한 정규화(lambda=2.0) + 7seeds × 5fold"},
