...
OUT_DIR  = f"{DATA_DIR}/submissions/sub_05/trial_022_multi_seed"
...
params = {"objective":"binary","metric":"auc","learning_rate":0.05,"num_leaves":64,
          ...
          "bagging_freq":5,"lambda_l1":0.5,"lambda_l2":0.5,"verbose":-1}
...
print(f"\n{'='*40}")
print(f"Multi-seed OOF AUC: {final_auc:.5f}  ({len(SEEDS)} seeds × {N_FOLDS} folds = {len(SEEDS)*N_FOLDS} models)")

np.save(f"{OUT_DIR}/oof_preds.npy", all_oof)
np.save(f"{OUT_DIR}/test_preds.npy", all_test)
pd.DataFrame({"id":test["id"],"Churn":all_test}).to_csv(f"{OUT_DIR}/trial_022_multi_seed.csv",index=False)
json.dump({"id":"022","status":"done","val_score":round(final_auc,5),
           "seeds":SEEDS,"notes":f"{len(SEEDS)} seeds × {N_FOLDS}-Fold = {len(SEEDS)*N_FOLDS} models"},
