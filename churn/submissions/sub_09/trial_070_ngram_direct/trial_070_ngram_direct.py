"""N-gram categorical features (bi-gram + tri-gram TE) + distribution + digit + ORIG stats + XGB multi-seed"""
import pandas as pd
import numpy as np
...
OUT_DIR = f"{DATA_DIR}/submissions/sub_09/trial_070_ngram_direct"
...
print("=== Building N-gram categorical features ===", flush=True)

# Top 6 most predictive categoricals
top6_cats = ["Contract", "InternetService", "PaymentMethod", "OnlineSecurity", "TechSupport", "PaperlessBilling"]

# Create bi-gram (2-way) and tri-gram (3-way) combination columns
ngram_cols = []

# Bi-grams: 6C2 = 15 combinations
for c1, c2 in combinations(top6_cats, 2):
    col_name = f"bigram_{c1[:4]}_{c2[:4]}"
    for df in [train, test]:
        df[col_name] = df[c1].astype(str) + "_" + df[c2].astype(str)
    ngram_cols.append(col_name)

print(f"  Bi-gram columns: {len([c for c in ngram_cols if c.startswith('bigram')])}", flush=True)

# Tri-grams: 6C3 = 20 combinations (NEW - not tried before)
for c1, c2, c3 in combinations(top6_cats, 3):
    col_name = f"trigram_{c1[:3]}_{c2[:3]}_{c3[:3]}"
    for df in [train, test]:
        df[col_name] = df[c1].astype(str) + "_" + df[c2].astype(str) + "_" + df[c3].astype(str)
    ngram_cols.append(col_name)

print(f"  Tri-gram columns: {len([c for c in ngram_cols if c.startswith('trigram')])}", flush=True)
print(f"  Total N-gram columns: {len(ngram_cols)}", flush=True)

# All categorical columns for TE = original cats + ngram cats
all_te_cols = cat_cols + ngram_cols
...
print(f"Total categorical features for TE: {len(all_te_cols)}", flush=True)
...
pd.DataFrame({"id": test["id"], "Churn": all_test}).to_csv(
    f"{OUT_DIR}/trial_070_ngram_direct.csv", index=False)
json.dump({
    "id": "070", "name": "ngram_direct",
    "strategy": "N-gram (bi+tri-gram) categorical TE + distribution + digit + ORIG stats + XGB 7-seed",
