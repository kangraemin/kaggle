"""TE std enriched: All 2-way combos + TE mean/std + digit features + ORIG stats (no distribution, no pseudo labels)"""
import pandas as pd
import numpy as np
...
OUT_DIR = f"{DATA_DIR}/submissions/sub_09/trial_074_te_std_enriched"
...
# XGB params (trial_066 base, slightly more regularized)
xgb_params = {
...
pd.DataFrame({"id": test["id"], "Churn": all_test}).to_csv(
    f"{OUT_DIR}/trial_074_te_std_enriched.csv", index=False)
json.dump({
    "id": "074", "name": "te_std_enriched",
    "strategy": "All 2-way combos (120) + TE mean/std + digit features + ORIG stats(mean+std) + freq enc + 7-seed XGB (no dist, no pseudo)",
