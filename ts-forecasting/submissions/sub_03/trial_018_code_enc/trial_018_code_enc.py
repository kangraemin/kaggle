"""Trial 018: code-level encoding 추가
- code 오버랩 100% → code, code×horizon 통계 test에서 완전 유효
- sub_code 오버랩 25.5% → stats 버림 (categorical split만 유지)
- 제외: feature_w/x/y/z (38% NaN), feature_at/by (9.2% NaN)
...
def compute_group_stats(df):
    s_code_h = df.groupby(['code', 'horizon'])['y_target'].agg(
        code_h_mean='mean', code_h_std='std'
    ).reset_index()
    s_code = df.groupby(['code'])['y_target'].agg(
    ...
    return s_code_h, s_code, s_subcat_h, s_horizon


def apply_group_stats(df, s_code_h, s_code, s_subcat_h, s_horizon):
    df = df.merge(s_code_h,   on=['code', 'horizon'],        how='left')
    df = df.merge(s_code,     on=['code'],                   how='left')
...
def get_feature_cols(df):
    ...
    enc = [c for c in df.columns if
           c.endswith('_mean') or c.endswith('_std')]
    return raw + enc
...
def main():
    ...
    print(f"\n[Trial 018] Cold-start val score: {score:.6f}")
    ...
    save_submission(test, model_full.predict(X_te), 'trial_018_code_enc',
