"""Trial 028: 고 MI feature 비선형 변환
- feature_bs, feature_bt (항상 음수) → inv_bs = -1/feature (역수 선형화)
- feature_am, feature_bw (0 이상) → log1p 변환
- feature_bx (양수) → log 변환
- MI 분석에서 발견: 선형 corr 0.09지만 MI 0.76+ 비선형 관계
- cold-start val 10% holdout, 3000라운드
"""
import gc
import sys
import psutil
...
def add_transforms(df):
    """고 MI feature 비선형 변환."""
    # 항상 음수 → 역수 (양수)
    for col in ['feature_bs', 'feature_bt']:
        if col in df.columns:
            df[f'inv_{col}'] = -1.0 / df[col].clip(upper=-1e-6)
    # 0 이상 → log1p
    for col in ['feature_am', 'feature_bw']:
        if col in df.columns:
            df[f'log_{col}'] = np.log1p(df[col].clip(lower=0))
    # 양수 → log
    if 'feature_bx' in df.columns:
        df['log_feature_bx'] = np.log(df['feature_bx'].clip(lower=1e-6))
    return df
...
def get_feature_cols(df):
    ...
    transformed = [c for c in df.columns if c.startswith('inv_') or c.startswith('log_')]
    return raw + enc + transformed
...
def main():
    ...
    tr  = add_transforms(tr)
    val = add_transforms(val)
    ...
    train_full = add_transforms(train_full)
    ...
    save_submission(test, model_full.predict(X_te), 'trial_028_feat_transform',
