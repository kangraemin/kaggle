"""Trial 040: cold-start val 5%→10%
- test가 89% unknown이니까 val도 unknown 비율 높이기
- trial_040(0.4515) 기반
"""
import gc
import sys
...
def main():
    ...
    np.random.seed(42)
    holdout_idx = np.random.choice(len(all_series), size=int(len(all_series) * 0.10), replace=False)
    ...
    save_submission(test, preds, 'trial_040_ts_val_improved',
