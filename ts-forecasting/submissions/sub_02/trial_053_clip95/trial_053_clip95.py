...
def compute_clip_bounds(df):
    ...
    for col in feat_cols:
        bounds[col] = (float(df[col].quantile(0.001)), float(df[col].quantile(0.95)))
    return bounds
...
def add_series_features(df, train_df):
    ...
    hi = float(train_df['y_target'].quantile(0.95))
...
def main():
    ...
    w_cap = float(train['weight'].quantile(0.95))
    ...
    save_submission(test, preds, 'trial_053_ts_val_improved',
