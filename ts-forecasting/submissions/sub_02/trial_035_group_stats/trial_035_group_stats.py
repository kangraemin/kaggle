"""Trial 035: unknown series에 group stats 추가
- val=100% known, test=89% unknown → unknown이 병목
- code_mean/std (100% 커버), subcat×horizon stats, horizon stats
- known series: series features 유지
- danger 자동 패치로 83EG83KQ 등 안전하게 처리
"""
import gc
import sys
import psutil
...
def add_group_stats(df, train_df):
    """unknown series를 위한 group 통계 (code, subcat×horizon, horizon)."""
    lo = float(train_df['y_target'].quantile(0.001))
    hi = float(train_df['y_target'].quantile(0.999))

    code_stats = train_df.groupby('code')['y_target'].agg(
        code_mean='mean', code_std='std'
    ).reset_index()
    subcat_stats = train_df.groupby(['sub_category', 'horizon'])['y_target'].agg(
        subcat_mean='mean', subcat_std='std'
    ).reset_index()
    h_stats = train_df.groupby('horizon')['y_target'].agg(
        horizon_mean='mean', horizon_std='std'
    ).reset_index()

    for stats, keys in [(code_stats, ['code']),
                        (subcat_stats, ['sub_category', 'horizon']),
                        (h_stats, ['horizon'])]:
        df = df.merge(stats, on=keys, how='left')

    for col in ['code_mean', 'code_std', 'subcat_mean', 'subcat_std', 'horizon_mean', 'horizon_std']:
        if col in df.columns:
            df[col] = df[col].clip(lower=lo if 'std' not in col else 0,
                                   upper=hi if 'std' not in col else (hi - lo))
    return df
...
def prepare_X(df):
    ...
    series_feats = [c for c in ['last_y_1', 'last_y_2', 'last_y_3', 'series_mean', 'series_std',
                                 'code_mean', 'code_std', 'subcat_mean', 'subcat_std',
...
def main():
    ...
    # group stats 추가 (unknown series용)
    print("Adding group stats...")
    train = add_group_stats(train, train)
    ...
    test = add_group_stats(test, train_full_for_feats)
    ...
    save_submission(test, preds, 'trial_035_ts_val_improved',
