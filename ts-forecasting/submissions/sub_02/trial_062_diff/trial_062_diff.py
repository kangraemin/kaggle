"""Trial 062: first-difference feature 추가
- last_y_1 - last_y_2 = 추세 방향 (올라가는 중 vs 내려가는 중)
- last_y_1 - series_mean = 평균 대비 현재 위치 (평균회귀 신호)
- trial_062(0.5912) 기반
"""
import gc
import sys
...
def add_series_features(df, train_df):
    ...
    result['diff_1_2']      = (result['last_y_1'] - result['last_y_2']).clip(lower=lo, upper=hi)
    result['dev_from_mean'] = (result['last_y_1'] - result['series_mean']).clip(lower=lo, upper=hi)
...
def prepare_X(df):
    ...
    series_feats = [c for c in [
        ...
        'trend_1_5', 'trend_1_10', 'diff_1_2', 'dev_from_mean', 'series_mean', 'series_std',
...
def main():
    ...
    save_submission(test, preds, 'trial_062_ts_val_improved',
