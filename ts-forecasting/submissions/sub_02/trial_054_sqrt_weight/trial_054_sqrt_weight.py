"""Trial 054: sqrt(weight) 변환
- clip 대신 sqrt → 더 smooth한 weight 압축 (13조 → 3.6M)
- trial_054(0.4915) 기반
"""
import gc
import sys
...
def main():
    ...
    # sqrt(weight) 변환
    train['weight'] = np.sqrt(train['weight'])
    print(f"Weight sqrt: max={train['weight'].max():.2e}")
    ...
    save_submission(test, preds, 'trial_054_ts_val_improved',
