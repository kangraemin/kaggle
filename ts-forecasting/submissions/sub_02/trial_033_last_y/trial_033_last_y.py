"""Trial 033: known series에 last_y feature 추가
- known test series (10.9%): 마지막 training y_target을 feature로 사용
- W4S29LF4 (std=142)같은 큰 y 시리즈 예측 개선 목표
- last_y도 클리핑 적용 (train y_target 범위로)
- unknown series: last_y = NaN (LightGBM이 처리)
"""
import gc
import sys
import psutil
...
def add_last_y(df, train_df):
    """known series에 마지막 training y_target 추가."""
    last_y = (train_df.sort_values(KEY + ['ts_index'])
              .groupby(KEY)['y_target'].last().reset_index()
              .rename(columns={'y_target': 'last_y'}))
    # last_y 클리핑 (train y_target 범위로)
    lo = float(train_df['y_target'].quantile(0.001))
    hi = float(train_df['y_target'].quantile(0.999))
    last_y['last_y'] = last_y['last_y'].clip(lower=lo, upper=hi)
    df = df.merge(last_y, on=KEY, how='left')  # unknown → NaN
    return df
...
def prepare_X(df):
    ...
    extra = ['last_y'] if 'last_y' in df.columns else []
    X = df[feat_cols + cat_cols + ['horizon', 'ts_index'] + extra].copy()
    for c in cat_cols:
...
def main():
    ...
    # last_y feature 추가 (known series만)
    train = add_last_y(train, train)
    ...
    # test known series에 last_y 추가 (train_full 기준)
    train_full_for_lasty = load_train()
    test = add_last_y(test, train_full_for_lasty)
    known_cnt = test['last_y'].notna().sum()
    print(f"Test known series with last_y: {known_cnt:,} ({known_cnt/len(test)*100:.1f}%)")
    del train_full_for_lasty
    gc.collect()
    X_te = prepare_X(test)
    ...
    save_submission(test, preds, 'trial_033_ts_val_improved',
