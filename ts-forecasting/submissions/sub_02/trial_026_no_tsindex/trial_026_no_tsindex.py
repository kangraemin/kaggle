"""Trial 026: ts_index 제거
- test ts_index(3602~4376)가 train 범위(1~3601) 완전 밖 → extrapolation
- ts_index 제거 후 raw feature + group stats만으로 일반화
- 나머지는 trial_021과 동일
"""
import gc
import sys
...
def prepare_X(df):
    ...
    X = df[feat_cols + cat_cols + ['horizon']].copy()
    for c in cat_cols:
...
def main():
    ...
    save_submission(test, model_full.predict(X_te), 'trial_026_tuned',
