"""Trial 064: val = cold-start 100% (time-split 제거)
- test가 89% unknown → val도 unknown만으로 구성해야 정직한 early stopping
- 지금까지 val = 85% known + 15% unknown → 왜곡됨
- trial_064(0.5912) 기반, weight^0.05 유지
"""
import gc
import sys
import psutil
...
def main():
    ...
    # weight^0.1 변환
    train['weight'] = train['weight'] ** 0.05
    print(f"Weight^0.1: max={train['weight'].max():.2e}")

    # Pure cold-start val: time-split 없이 unknown series만 평가
    # test가 89% unknown → val도 unknown만으로 구성해야 정직한 early stopping
    ...
    tr  = train[~is_holdout]   # holdout 제외한 전체 (time-split 없음)
    val = train[is_holdout]    # holdout series만 val
    print(f"Pure cold-start val: train={len(tr):,}, val={len(val):,} ({is_holdout.mean()*100:.1f}% holdout)")
    ...
    save_submission(test, preds, 'trial_064_ts_val_improved',
