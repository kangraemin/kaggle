"""Trial 049: AR 예측값을 feature로
- 시리즈별 AR(1) 계수 계산: ar_coef = corr(y(t), y(t-1))
- ar_pred_1 = ar_coef × last_y_1 + (1-ar_coef) × series_mean  (1-step AR 예측)
- ar_pred_k = ar_coef^k × last_y_1 + (1-ar_coef^k) × series_mean  (k-step)
- 이론적 최적 예측값을 feature로 → 모델 학습 쉬워짐
- trial_049(0.4887) 기반
"""
import gc
import sys
...
def compute_ar_coefs(train_df):
    """시리즈별 AR(1) 계수 계산."""
    sorted_tr = train_df.sort_values(KEY + ['ts_index'])
    sorted_tr['lag1'] = sorted_tr.groupby(KEY)['y_target'].shift(1)
    valid = sorted_tr.dropna(subset=['lag1'])

    def safe_corr(g):
        if len(g) < 3:
            return np.nan
        return float(g['y_target'].corr(g['lag1']))

    ar = valid.groupby(KEY).apply(safe_corr).reset_index()
    ar.columns = list(KEY) + ['ar_coef']
    ar['ar_coef'] = ar['ar_coef'].clip(lower=-1, upper=1).fillna(0)
    return ar
...
def add_series_features(df, train_df):
    ...
    # AR 계수 + AR 예측값 추가
    ar = compute_ar_coefs(train_df)
    result = result.merge(ar, on=KEY, how='left')
    result['ar_coef'] = result['ar_coef'].fillna(0)

    # AR 예측: ar_pred_k = ar_coef^k * last_y_1 + (1-ar_coef^k) * series_mean
    for k in [1, 3, 5, 10, 20]:
        ar_k = result['ar_coef'] ** k
        result[f'ar_pred_{k}'] = ar_k * result['last_y_1'] + (1 - ar_k) * result['series_mean']
        result[f'ar_pred_{k}'] = result[f'ar_pred_{k}'].clip(lower=lo, upper=hi)
...
def prepare_X(df):
    ...
    series_feats = [c for c in [
        ...
        'ar_coef', 'ar_pred_1', 'ar_pred_3', 'ar_pred_5', 'ar_pred_10', 'ar_pred_20',
...
def main():
    ...
    save_submission(test, preds, 'trial_049_ts_val_improved',
