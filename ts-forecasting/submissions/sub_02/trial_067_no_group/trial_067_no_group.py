"""Trial 067: group stats 제거
- 가설: subcat_mean 등이 unknown series 예측을 non-zero로 만들어 0점 유발
- sub_01은 group stats 없이 0.1499 달성
- known series: series features(last_y, lags) 유지
- unknown series: raw features만 → near-0 예측 (0점 방지)
- weight^0.05 유지
"""
import gc
import sys
...
def prepare_X(df):
    ...
    series_feats = [c for c in [
        ...
        # group stats 제거 (unknown series 예측 non-zero → 0점 유발)
    ] if c in df.columns]
...
def main():
    ...
    # group stats 제거 (unknown series 예측 non-zero → 0점 유발 가설)
    ...
    save_submission(test, preds, 'trial_067_ts_val_improved',
