"""Trial 048: series median + percentiles 추가
- series_median, series_q25, series_q75 (이상치 robust 통계)
- trial_048(0.4887) 기반
"""
import gc
import sys
...
def add_series_features(df, train_df):
    ...
    stats = grp.agg(
        ...
        series_q25=lambda x: x.quantile(0.25),
        series_q75=lambda x: x.quantile(0.75),
    ).reset_index()
    for col in ['series_mean', 'series_median', 'series_q25', 'series_q75']:
        stats[col] = stats[col].clip(lower=lo, upper=hi)
    stats['series_std'] = stats['series_std'].clip(lower=0, upper=(hi - lo))
    stats['series_iqr'] = (stats['series_q75'] - stats['series_q25']).clip(lower=0)
...
def prepare_X(df):
    ...
    series_feats = [c for c in [
        ...
        'trend_1_5', 'trend_1_10', 'series_mean', 'series_std', 'series_median', 'series_q25', 'series_q75', 'series_iqr',
...
def main():
    ...
    save_submission(test, preds, 'trial_048_ts_val_improved',
