"""Trial 039: mixed val split
- val = ts_index>2880 (known) + cold-start 5% holdout (unknown)
- 현재 val이 100% known → unknown series early stopping 못 함
- mixed val로 both known/unknown 밸런스 맞추기
- trial_039 features 유지
"""
import gc
import sys
import psutil
...
def prepare_X(df):
    ...
    series_feats = [c for c in [
        ...
        'code_mean', 'code_std', 'subcat_mean', 'subcat_std', 'horizon_mean', 'horizon_std'
    ] if c in df.columns]
...
def main():
    ...
    np.random.seed(42)
    holdout_idx = np.random.choice(len(all_series), size=int(len(all_series) * 0.05), replace=False)
    ...
    save_submission(test, preds, 'trial_039_ts_val_improved',
