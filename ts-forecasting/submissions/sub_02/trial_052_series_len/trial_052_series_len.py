"""Trial 052: series_length feature 추가
- 관측값 수 → series_mean 신뢰도 표현
- 짧은 시리즈(n=1)는 series_mean 불신뢰, 긴 시리즈는 신뢰
- weight clipping 유지 (99.9%)
- trial_052(0.4904) 기반
"""
import gc
import sys
...
def add_series_features(df, train_df):
    ...
    stats['series_log_count'] = np.log1p(stats['series_count'])
...
def prepare_X(df):
    ...
    series_feats = [c for c in [
        ...
        'trend_1_5', 'trend_1_10', 'series_mean', 'series_std', 'series_count', 'series_log_count',
...
def main():
    ...
    w_cap = float(train['weight'].quantile(0.999))
    ...
    save_submission(test, preds, 'trial_052_ts_val_improved',
