"""Trial 044: test series raw feature stats
- unknown test series의 raw feature mean/std를 series-level feature로 추가
- ex) test에서 (code, sub_code, sub_category, horizon) 그룹별 feature_v mean
- unknown series의 '타입' 추정 → 더 정확한 y_target 예측
- trial_044(0.4873) 기반 (cold-start 15%)
"""
import gc
import sys
import psutil
...
def prepare_X(df):
    ...
    test_smean_feats = [c for c in df.columns if c.startswith('test_smean_')]
    X = df[feat_cols + cat_cols + ['horizon', 'ts_index'] + series_feats + test_smean_feats].copy()
    for c in cat_cols:
...
def main():
    ...
    # train series raw feature stats
    top_raw = ['feature_v', 'feature_cg', 'feature_a', 'feature_cf', 'feature_aq',
               'feature_ce', 'feature_as', 'feature_ai', 'feature_h', 'feature_al']
    for feat in top_raw:
        if feat in train.columns:
            train[f'test_smean_{feat}'] = train.groupby(KEY)[feat].transform('mean')
    ...
    # test series raw feature stats (unknown series용 '타입' 추정)
    # 각 test series(KEY)의 raw feature 평균 → 시리즈 특성 proxy
    print("Computing test series raw feature stats...")
    top_raw = ['feature_v', 'feature_cg', 'feature_a', 'feature_cf', 'feature_aq',
               'feature_ce', 'feature_as', 'feature_ai', 'feature_h', 'feature_al']
    for feat in top_raw:
        if feat in test.columns:
            series_feat_mean = test.groupby(KEY)[feat].transform('mean')
            test[f'test_smean_{feat}'] = series_feat_mean
    ...
    save_submission(test, preds, 'trial_044_ts_val_improved',
