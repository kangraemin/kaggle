"""
Trial 011: Slow XGBoost (lr=0.01, 15k rounds) + sklearn Multiclass TE on ALL 171 Pairwise
- Base: trial_010/trial_008b architecture (171 pairwise factorize, Deotte binary, orig TE)
- Key change 1: lr 0.1 → 0.01, n_estimators 2000 → 15000, early_stopping 50 → 200
- Key change 2: sklearn TargetEncoder(multiclass) on ALL 171 pairwise (513 TE features)
- Single seed (seed=42, 5 folds = 5 models) — ~2-3 hours
- Hypothesis: Chris Deotte/Mahog use lr=0.01 + many rounds → closing the gap
"""
...
for fold, (train_idx, val_idx) in enumerate(skf.split(train, y)):
    ...
    # sklearn TargetEncoder with target_type="multiclass" outputs (n, n_classes * n_features)
...
results = {
    "trial": "trial_011_slow_xgb_deeper_trees",
