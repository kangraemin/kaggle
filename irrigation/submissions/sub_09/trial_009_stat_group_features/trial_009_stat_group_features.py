"""
Trial 009: Statistical Group Features + Orig TE on Numerics + Extended XGB Iterations
- Statistical group features: (binned_numeric x categorical) -> mean/std/freq per label class
  - 11 NUMS x 8 CATS = 88 pairs, cardinality-filtered
  - Smoothed class fractions (alpha=10), in-fold computation
  - ~352 new feature columns
- Original TE extended to ALL 19 features (CATS + NUMS): 57 prior features (was 24)
- Extended XGB: 15000 rounds, early_stopping 500
- Everything else preserved from trial_008
"""
...
# ============ Original Data TE - Extended to CATS + NUMS ============
orig_te_cols = []
...
# NEW: NUMS TE - bin each numeric then compute class fractions from original
NUM_TE_BINS = 10
for col in NUMS:
    bin_col = f"{col}_te_bin"
    all_vals = pd.concat([train[col], original[col]])
    bins_edges = pd.qcut(all_vals, q=NUM_TE_BINS, duplicates="drop", retbins=True)[1]
    for df in [train, test, original]:
        df[bin_col] = pd.cut(df[col], bins=bins_edges, labels=False, include_lowest=True).fillna(0).astype(int)
    for cls in range(3):
        col_name = f"orig_te_{col}_cls{cls}"
        mean_val = pd.Series(y_orig == cls).groupby(original[bin_col]).mean()
        global_mean = (y_orig == cls).mean()
        train[col_name] = train[bin_col].map(mean_val).fillna(global_mean)
        test[col_name] = test[bin_col].map(mean_val).fillna(global_mean)
        original[col_name] = original[bin_col].map(mean_val).fillna(global_mean)
        orig_te_cols.append(col_name)
    for df in [train, test, original]:
        df.drop(columns=[bin_col], inplace=True, errors="ignore")

print(f"Original data TE: {len(orig_te_cols)} cols (CATS + NUMS)")
...
# Final feature list
drop_cols_set = {"id", "Irrigation_Need"}
features = [c for c in train.columns if c not in drop_cols_set]
...
print(f"Total features (before fold TE + stat group): {len(features)}")
...
# ============ Statistical Group Features Helper ============

def compute_stat_group_features(
    df_source,
    y_source,
    df_apply_list,
    cat_cols_enc,
    ...
    """
    Kospintr-style: for each (num_bin x cat) pair, compute smoothed class fractions + freq.
    """
    n_classes = 3
    global_counts = np.bincount(y_source, minlength=n_classes)
    global_probs = global_counts / len(y_source)

    new_feat_names = []
    results = {i: {} for i in range(len(df_apply_list))}

    # Compute bin edges on source
    num_bins_map = {}
    for num_col in num_col_list:
        vals = df_source[num_col].values
        bins_edges = np.percentile(vals, np.linspace(0, 100, n_bins + 1))
        bins_edges = np.unique(bins_edges)
        if len(bins_edges) < 3:
            continue
        num_bins_map[num_col] = bins_edges

    for num_col, bins_edges in num_bins_map.items():
        src_binned = np.digitize(df_source[num_col].values, bins_edges[1:-1])

        for cat_col in cat_cols_enc:
            src_cat = df_source[cat_col].values

            group_key_src = src_binned * 1000 + src_cat

            group_stats = {}
            unique_keys = np.unique(group_key_src)
            for gk in unique_keys:
                mask = group_key_src == gk
                n_group = mask.sum()
                class_counts = np.bincount(y_source[mask], minlength=n_classes)
                smoothed = (class_counts + alpha * global_probs) / (n_group + alpha)
                group_stats[gk] = np.append(smoothed, n_group)

            for df_idx, (df_apply, _) in enumerate(df_apply_list):
                apply_binned = np.digitize(df_apply[num_col].values, bins_edges[1:-1])
                apply_cat = df_apply[cat_col].values
                apply_key = apply_binned * 1000 + apply_cat

                for cls in range(n_classes):
                    feat_name = f"sg_{num_col}_x_{cat_col}_cls{cls}"
                    feat_vals = np.array([
                        group_stats[k][cls] if k in group_stats else global_probs[cls]
                        for k in apply_key
                    ])
                    results[df_idx][feat_name] = feat_vals
                    if df_idx == 0:
                        new_feat_names.append(feat_name)

                feat_name_freq = f"sg_{num_col}_x_{cat_col}_freq"
                feat_vals_freq = np.array([
                    group_stats[k][n_classes] if k in group_stats else 0.0
                    for k in apply_key
                ], dtype=float)
                results[df_idx][feat_name_freq] = feat_vals_freq / len(df_source)
                if df_idx == 0:
                    new_feat_names.append(feat_name_freq)

    return new_feat_names, results
...
sg_feat_names_global = []

for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
    print(f"\n{'='*50} Fold {fold} {'='*50}")
    y_tr, y_val = y[train_idx], y[val_idx]

    # Full original data append
    X_tr_base = pd.concat([X.iloc[train_idx], X_orig], ignore_index=True)
    ...
    # --- Statistical Group Features (NEW) ---
    print(f"  Computing statistical group features...")
    X_tr_train_only = X.iloc[train_idx]
    y_tr_train_only = y_tr

    df_apply_list = [
        (X_tr_base, "train+orig"),
        (X_val, "val"),
        (X_test_fold, "test"),
    ]

    sg_feat_names, sg_results = compute_stat_group_features(
        df_source=X_tr_train_only,
        y_source=y_tr_train_only,
        df_apply_list=df_apply_list,
        cat_cols_enc=CATS,
        num_col_list=NUMS,
        n_bins=20,
        alpha=10,
    ...
    if sg_feat_names:
        X_tr_base = pd.concat([X_tr_base, pd.DataFrame(sg_results[0], index=X_tr_base.index)], axis=1)
        X_val = pd.concat([X_val, pd.DataFrame(sg_results[1], index=X_val.index)], axis=1)
        X_test_fold = pd.concat([X_test_fold, pd.DataFrame(sg_results[2], index=X_test_fold.index)], axis=1)
        if fold == 0:
            sg_feat_names_global = sg_feat_names
        print(f"  Stat group features: {len(sg_feat_names)} cols")

    # --- sklearn TargetEncoder(multiclass) ---
    ...
    for batch_start in range(0, len(te_source_cols), batch_size):
        ...
        te.fit(X_tr_base[batch_cols].values, y_tr_full)

        tr_encoded = te.transform(X_tr_base[batch_cols].values)
    ...
    X_tr_base = pd.concat([X_tr_base, pd.DataFrame(tr_te_dict, index=X_tr_base.index)], axis=1)
    ...
    fold_features = list(X_tr_base.columns)
    print(f"  Fold features: {len(fold_features)} (stat_group: {len(sg_feat_names)}, TE: {len(te_new_col_names)})")

    # --- XGBoost ---
    dtrain = xgb.DMatrix(X_tr_base[fold_features], label=y_tr_full, weight=sample_w)
...
results = {
    "trial": "trial_009_stat_group_features",
