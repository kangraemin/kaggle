"""
Trial 010: Multi-Seed XGB Averaging on trial_008b architecture
- Base: trial_008b (171 pairwise factorize, 24 multiclass TE on cats, Deotte binary, orig TE)
- Key change: 5 seeds x 5 folds = 25 total models
- Seeds: [42, 123, 456, 789, 2024]
- OOF/Test: mean of 5 per-seed arrays
- Threshold optimization on averaged OOF
...
# Accumulate OOF/test across seeds
all_oof_arrays = []  # list of (n_train, 3) arrays, one per seed
all_test_arrays = []  # list of (n_test, 3) arrays, one per seed
...
for seed_idx, seed in enumerate(SEEDS):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train, y)):
        ...
        print(f"  XGB  bal_acc fold {fold}: {fold_bal:.6f}")
        del model_xgb; gc.collect()

    seed_bal = balanced_accuracy_score(y, oof_xgb_arr.argmax(1))
    print(f"\nSeed {seed} OOF bal_acc: {seed_bal:.6f}")
    all_oof_arrays.append(oof_xgb_arr)
    all_test_arrays.append(test_xgb_arr)

# ============ Average across seeds ============

oof_avg = np.mean(all_oof_arrays, axis=0)
test_avg = np.mean(all_test_arrays, axis=0)

raw_bal_acc = balanced_accuracy_score(y, oof_avg.argmax(1))
print(f"\nMulti-seed averaged OOF bal_acc (raw): {raw_bal_acc:.6f}")

for i, (seed, oof) in enumerate(zip(SEEDS, all_oof_arrays)):
    print(f"  Seed {seed}: {balanced_accuracy_score(y, oof.argmax(1)):.6f}")
...
results = {
    ...
    "per_seed_bal_acc": {
        str(seed): round(balanced_accuracy_score(y, oof.argmax(1)), 6)
        for seed, oof in zip(SEEDS, all_oof_arrays)
    },
    "n_base_features": len(base_features),
    "n_te_per_fold": 24,
    "n_total_features": len(base_features) + 24,
}
