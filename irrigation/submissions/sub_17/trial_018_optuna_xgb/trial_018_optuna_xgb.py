"""
Trial 018: Optuna XGB Hyperparameter Search
- Base: trial_013 architecture (750 features, orig append, bias tuning)
- No new features (digit/magic/logit hurt public)
- Optuna searches XGB params for 50 trials
- Best params → 5-seed final run
"""
...
def run_cv(params, seed=42, n_folds=5, verbose=False):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train, y)):
        ...
        model = xgb.XGBClassifier(
            ...
            random_state=seed,
            tree_method="hist",
            max_bin=params.get("max_bin", 1024),
            verbosity=0,
        )
        model.fit(
            X_tr_np, y_combined,
    ...
    raw_score = balanced_accuracy_score(y, oof.argmax(1))
    _, biased_score = tune_bias(oof, y)
    return raw_score, biased_score, oof
...
def objective(trial):
    params = {
        ...
        "orig_weight": trial.suggest_float("orig_weight", 0.1, 0.5),
        "max_bin": trial.suggest_categorical("max_bin", [512, 1024, 2048]),
    }

    raw, biased, _ = run_cv(params, seed=42, n_folds=5)
    print(f"  Trial {trial.number}: raw={raw:.6f}, biased={biased:.6f}, params={json.dumps({k: round(v,4) if isinstance(v,float) else v for k,v in params.items()})}")
    return biased
...
# ============ Final run with best params + 5 seeds ============

print("\n" + "="*60)
print("Final run with best params + 5 seeds")
print("="*60)
...
for si, seed in enumerate(SEEDS):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train, y)):
        ...
        sw_synth = compute_sample_weight("balanced", y_tr)
        sw_orig = np.full(N_ORIG, best_params.get("orig_weight", 0.35))
        sw_combined = np.concatenate([sw_synth, sw_orig])
        y_combined = np.concatenate([y_tr, orig_y])
        ...
        model = xgb.XGBClassifier(
            ...
            random_state=seed,
            tree_method="hist",
            max_bin=best_params.get("max_bin", 1024),
            verbosity=0,
        )
        model.fit(
            X_tr_np, y_combined,
