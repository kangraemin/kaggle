...
**같은 sub에 넣을 수 있는 것**: 같은 base 노트북/파이프라인의 파라미터 변경, 피처 추가, 후처리 변경.

**새 sub을 만들어야 하는 것**: 다른 base 노트북 fork, 완전히 다른 모델 아키텍처, 다른 사람의 노트북 fork.

예시:
- 0.912 fork에서 PCA 바꾸기 → 같은 sub
- 0.926 fork로 전환 → **새 sub**
- CNN 블렌딩 추가 → **새 sub**

이 규칙을 어기면 한 sub에 서로 다른 파이프라인의 결과가 섞여서 reflection이 무의미해진다.
